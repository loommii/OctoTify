/**
 * This file was auto-generated by openapi-typescript.
 * Do not make direct changes to the file.
 */

export interface paths {
    "/auth/login": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get?: never;
        put?: never;
        /**
         * 用户登录
         * @description 使用用户名和密码登录系统，登录成功后返回 Access Token 和 Refresh Token。
         *     Access Token 有效期 1 小时，用于调用需要认证的接口。
         *     Refresh Token 有效期 7 天，用于刷新 Access Token。
         *     ## 密码要求
         *     - 8-128 个字符
         *     - 必须包含小写字母
         *     - 必须包含大写字母
         *     - 必须包含数字
         *     ## 使用场景
         *     1. 用户首次访问系统时进行登录
         *     2. Refresh Token 过期后重新登录
         *     ## 注意事项
         *     - 登录成功后请妥善保存 Access Token 和 Refresh Token
         *     - Access Token 过期后请使用 Refresh Token 调用刷新接口
         *     - 连续多次登录失败不会锁定账户
         *     ## 错误码说明
         *     - 110200: 用户名或密码错误
         *     - 110201: 登录失败
         */
        post: {
            parameters: {
                query?: never;
                header?: never;
                path?: never;
                cookie?: never;
            };
            /** @description 登录请求参数 */
            requestBody: {
                content: {
                    "application/json": components["schemas"]["octotify_internal_handler_dto.LoginReq"];
                };
            };
            responses: {
                /** @description 登录成功，返回认证信息 */
                200: {
                    headers: {
                        [name: string]: unknown;
                    };
                    content: {
                        "application/json": components["schemas"]["octotify_pkg_response.Response"] & {
                            data?: components["schemas"]["octotify_internal_handler_dto.AuthResp"];
                        };
                    };
                };
            };
        };
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/auth/logout": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get?: never;
        put?: never;
        /**
         * 退出登录
         * @description 撤销当前用户的所有 Refresh Token，退出登录后需要重新登录才能访问系统。
         *     ## 注意事项
         *     - Access Token 在过期前仍有效（JWT 无状态特性，最长 1 小时后自动失效）
         *     - 前端需主动清除本地存储的 Access Token 和 Refresh Token
         *     - 退出登录后，所有使用旧 Refresh Token 的刷新请求都会失败
         *     ## 错误码说明
         *     - 100001: 未提供认证令牌或认证令牌无效或已过期
         *     - 110304: 退出登录失败
         */
        post: {
            parameters: {
                query?: never;
                header?: never;
                path?: never;
                cookie?: never;
            };
            requestBody?: never;
            responses: {
                /** @description 退出成功 */
                200: {
                    headers: {
                        [name: string]: unknown;
                    };
                    content: {
                        "application/json": components["schemas"]["octotify_pkg_response.Response"];
                    };
                };
            };
        };
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/auth/refresh": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get?: never;
        put?: never;
        /**
         * 刷新令牌
         * @description 使用 Refresh Token 获取新的 Access Token 和 Refresh Token。
         *     每次刷新都会返回新的 Access Token 和 Refresh Token，旧 Refresh Token 继续有效。
         *     ## 使用场景
         *     1. Access Token 过期（1 小时后）时调用此接口
         *     2. 前端应在请求收到 401 响应时自动调用此接口
         *     ## 注意事项
         *     - Refresh Token 有效期 7 天，过期后需重新登录
         *     - 刷新成功后请更新本地存储的 Access Token 和 Refresh Token
         *     - 如果 Refresh Token 已被撤销（如退出登录、修改密码），则无法刷新
         *     ## 错误码说明
         *     - 110300: 刷新令牌无效
         *     - 110301: 刷新令牌已撤销
         *     - 110302: 刷新令牌已过期
         *     - 110303: 刷新令牌失败
         */
        post: {
            parameters: {
                query?: never;
                header?: never;
                path?: never;
                cookie?: never;
            };
            /** @description 刷新令牌请求参数 */
            requestBody: {
                content: {
                    "application/json": components["schemas"]["octotify_internal_handler_dto.RefreshReq"];
                };
            };
            responses: {
                /** @description 刷新成功 */
                200: {
                    headers: {
                        [name: string]: unknown;
                    };
                    content: {
                        "application/json": components["schemas"]["octotify_pkg_response.Response"] & {
                            data?: components["schemas"]["octotify_internal_handler_dto.AuthResp"];
                        };
                    };
                };
            };
        };
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/channel-types": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /**
         * 获取渠道类型元数据
         * @description 获取系统支持的所有推送渠道类型及其配置字段定义，用于前端动态渲染创建渠道表单。
         *     ## 使用场景
         *     1. 打开创建渠道页面时获取支持的渠道类型列表
         *     2. 根据 config_fields 动态生成配置表单
         *     ## 返回数据说明
         *     每个渠道类型包含：
         *     - type: 渠道类型标识（如 feishu, dingtalk, wechat 等）
         *     - name: 渠道类型名称（如 飞书, 钉钉, 企业微信 等）
         *     - description: 渠道类型描述
         *     - config_fields: 配置字段定义列表，用于前端动态表单渲染
         *     ## 错误码说明
         *     - 100001: 未提供认证令牌
         */
        get: {
            parameters: {
                query?: never;
                header?: never;
                path?: never;
                cookie?: never;
            };
            requestBody?: never;
            responses: {
                /** @description 获取成功 */
                200: {
                    headers: {
                        [name: string]: unknown;
                    };
                    content: {
                        "application/json": components["schemas"]["octotify_pkg_response.Response"] & {
                            data?: components["schemas"]["octotify_internal_handler_dto.ChannelTypeMeta"][];
                        };
                    };
                };
            };
        };
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/channels": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /**
         * 查看推送渠道列表
         * @description 分页查询当前用户的所有推送渠道列表，返回渠道基本信息。
         *     ## 使用场景
         *     1. 渠道管理页面展示渠道列表
         *     2. 下拉选择器获取渠道选项
         *     ## 注意事项
         *     - 列表按创建时间倒序排列
         *     - 已删除的渠道不会出现在列表中
         *     ## 错误码说明
         *     - 100001: 未提供认证令牌
         */
        get: {
            parameters: {
                query?: {
                    /** @description 页码，从 1 开始，默认 1 */
                    page?: number;
                    /** @description 每页条数，默认 20，最大 100 */
                    page_size?: number;
                };
                header?: never;
                path?: never;
                cookie?: never;
            };
            requestBody?: never;
            responses: {
                /** @description 查询成功 */
                200: {
                    headers: {
                        [name: string]: unknown;
                    };
                    content: {
                        "application/json": components["schemas"]["octotify_pkg_response.Response"] & {
                            data?: components["schemas"]["octotify_pkg_response.PageResult"] & {
                                list?: components["schemas"]["octotify_internal_handler_dto.ChannelDTO"][];
                            };
                        };
                    };
                };
            };
        };
        put?: never;
        /**
         * 创建推送渠道
         * @description 创建一个新的推送渠道，配置渠道类型、名称和凭证信息。
         *     ## 支持的渠道类型
         *     - **wechat**: 企业微信群机器人
         *     - **telegram**: Telegram Bot
         *     - **dingtalk**: 钉钉群机器人
         *     - **email**: 邮件推送
         *     - **webhook**: 自定义 Webhook
         *     - **feishu**: 飞书自定义机器人
         *     ## 使用场景
         *     1. 配置企业微信群机器人
         *     2. 配置 Telegram Bot
         *     3. 配置钉钉群机器人
         *     ## 注意事项
         *     - 不同渠道类型需要不同的 Config 配置
         *     - 创建后默认启用状态
         *     - 建议创建后使用测试接口验证配置
         *     ## 错误码说明
         *     - 100001: 未提供认证令牌
         *     - 120200: 渠道类型不支持
         *     - 120201: 渠道名称已存在
         *     - 120202: 创建渠道失败
         */
        post: {
            parameters: {
                query?: never;
                header?: never;
                path?: never;
                cookie?: never;
            };
            /** @description 创建推送渠道请求参数 */
            requestBody: {
                content: {
                    "application/json": components["schemas"]["octotify_internal_handler_dto.CreateChannelReq"];
                };
            };
            responses: {
                /** @description 创建成功 */
                200: {
                    headers: {
                        [name: string]: unknown;
                    };
                    content: {
                        "application/json": components["schemas"]["octotify_pkg_response.Response"] & {
                            data?: components["schemas"]["octotify_internal_handler_dto.ChannelDTO"];
                        };
                    };
                };
            };
        };
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/channels/wechat-clawbot/bind": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get?: never;
        put?: never;
        /**
         * 发起微信ClawBot扫码绑定
         * @description 获取微信ClawBot绑定二维码，用户扫码后完成绑定。
         *     ## 使用场景
         *     1. 创建微信ClawBot渠道时发起绑定
         *     2. 重新绑定微信账号
         *     ## 注意事项
         *     - 二维码有效期由 iLink 平台控制，过期后需重新获取
         *     - 前端需保存返回的 qrcode 值，用于后续调用 /bind/status 查询状态
         *     ## 错误码说明
         *     - 100001: 未提供认证令牌
         *     - 110901: 获取二维码失败
         */
        post: {
            parameters: {
                query?: never;
                header?: never;
                path?: never;
                cookie?: never;
            };
            requestBody?: never;
            responses: {
                /** @description 获取成功，返回 qrcode_url 和 qrcode */
                200: {
                    headers: {
                        [name: string]: unknown;
                    };
                    content: {
                        "application/json": components["schemas"]["octotify_pkg_response.Response"] & {
                            data?: components["schemas"]["octotify_internal_handler_dto.StartBindResp"];
                        };
                    };
                };
            };
        };
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/channels/wechat-clawbot/bind/status": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get?: never;
        put?: never;
        /**
         * 查询微信ClawBot绑定状态
         * @description 通过 qrcode 调用 iLink API 查询绑定状态（长轮询，40s 超时）。
         *     ## 工作原理
         *     1. 前端传 qrcode 字符串给后端
         *     2. 后端直接调用 iLink API 查询状态（iLink 本身是长轮询设计，40s 超时）
         *     3. iLink 返回状态时立即返回给前端
         *     4. 如果 iLink 超时，返回 pending，前端可重新发起请求
         *     ## 返回状态
         *     - pending: 仍在等待中（超时返回）
         *     - scanned: 用户已扫码，待确认
         *     - confirmed: 绑定成功，返回凭证
         *     - expired: 二维码已过期
         *     ## 错误码说明
         *     - 100001: 未提供认证令牌
         *     - 100000: 请求参数格式错误
         */
        post: {
            parameters: {
                query?: never;
                header?: never;
                path?: never;
                cookie?: never;
            };
            /** @description 查询绑定状态请求参数 */
            requestBody: {
                content: {
                    "application/json": components["schemas"]["octotify_internal_handler_dto.GetBindStatusReq"];
                };
            };
            responses: {
                /** @description 查询成功，返回绑定状态 */
                200: {
                    headers: {
                        [name: string]: unknown;
                    };
                    content: {
                        "application/json": components["schemas"]["octotify_pkg_response.Response"] & {
                            data?: components["schemas"]["octotify_internal_handler_dto.BindStatusResp"];
                        };
                    };
                };
            };
        };
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/channels/{id}": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /**
         * 查看渠道详情
         * @description 查询指定推送渠道的详细信息，包括渠道类型、名称、配置和状态。
         *     ## 使用场景
         *     1. 渠道详情页展示
         *     2. 查看渠道配置信息
         *     ## 注意事项
         *     - 只能查看自己创建的渠道
         *     - Config 字段包含渠道的完整配置信息
         *     ## 错误码说明
         *     - 100001: 未提供认证令牌
         *     - 120203: 渠道不存在
         *     - 120204: 无权限查看
         */
        get: {
            parameters: {
                query?: never;
                header?: never;
                path: {
                    /** @description 渠道ID，在路径中传递 */
                    id: number;
                };
                cookie?: never;
            };
            requestBody?: never;
            responses: {
                /** @description 查询成功 */
                200: {
                    headers: {
                        [name: string]: unknown;
                    };
                    content: {
                        "application/json": components["schemas"]["octotify_pkg_response.Response"] & {
                            data?: components["schemas"]["octotify_internal_handler_dto.ChannelDTO"];
                        };
                    };
                };
            };
        };
        /**
         * 编辑推送渠道
         * @description 编辑推送渠道的名称和配置信息，不影响已推送的消息。
         *     ## 使用场景
         *     1. 修改渠道名称使其更易识别
         *     2. 更新渠道配置（如更换 Webhook 地址）
         *     ## 注意事项
         *     - 只能编辑自己创建的渠道
         *     - 名称不能与其他渠道重复
         *     - 修改配置后建议重新测试
         *     ## 错误码说明
         *     - 100001: 未提供认证令牌
         *     - 120200: 渠道类型不支持
         *     - 120201: 渠道名称已存在
         *     - 120203: 渠道不存在
         *     - 120204: 无权限编辑
         */
        put: {
            parameters: {
                query?: never;
                header?: never;
                path: {
                    /** @description 渠道ID，在路径中传递 */
                    id: number;
                };
                cookie?: never;
            };
            /** @description 编辑推送渠道请求参数 */
            requestBody: {
                content: {
                    "application/json": components["schemas"]["octotify_internal_handler_dto.UpdateChannelReq"];
                };
            };
            responses: {
                /** @description 编辑成功 */
                200: {
                    headers: {
                        [name: string]: unknown;
                    };
                    content: {
                        "application/json": components["schemas"]["octotify_pkg_response.Response"];
                    };
                };
            };
        };
        post?: never;
        /**
         * 删除推送渠道
         * @description 软删除指定推送渠道，同时解除与所有来源的关联关系。删除后渠道和所有关联数据不可恢复。
         *     ## 使用场景
         *     1. 永久删除不再使用的渠道
         *     2. 清理测试数据
         *     ## 注意事项
         *     - 删除是软删除，数据库中标记为已删除状态
         *     - 删除后该渠道与所有来源的绑定关系解除
         *     - 只能删除自己创建的渠道
         *     ## 错误码说明
         *     - 100001: 未提供认证令牌
         *     - 120203: 渠道不存在
         *     - 120204: 无权限操作
         *     - 120208: 删除渠道失败
         */
        delete: {
            parameters: {
                query?: never;
                header?: never;
                path: {
                    /** @description 渠道ID，在路径中传递 */
                    id: number;
                };
                cookie?: never;
            };
            requestBody?: never;
            responses: {
                /** @description 删除成功 */
                200: {
                    headers: {
                        [name: string]: unknown;
                    };
                    content: {
                        "application/json": components["schemas"]["octotify_pkg_response.Response"];
                    };
                };
            };
        };
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/channels/{id}/disable": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get?: never;
        /**
         * 停用推送渠道
         * @description 停用指定推送渠道，停用后该渠道不再接收消息推送。
         *     ## 使用场景
         *     1. 临时停止某个渠道的消息推送
         *     2. 渠道维护期间停用
         *     ## 注意事项
         *     - 停用后推送到该渠道的消息会失败
         *     - 已推送的历史消息不受影响
         *     - 可以随时重新启用
         *     - 只能停用自己创建的渠道
         *     ## 错误码说明
         *     - 100001: 未提供认证令牌
         *     - 120203: 渠道不存在
         *     - 120204: 无权限操作
         *     - 120205: 渠道已停用
         */
        put: {
            parameters: {
                query?: never;
                header?: never;
                path: {
                    /** @description 渠道ID，在路径中传递 */
                    id: number;
                };
                cookie?: never;
            };
            requestBody?: never;
            responses: {
                /** @description 停用成功 */
                200: {
                    headers: {
                        [name: string]: unknown;
                    };
                    content: {
                        "application/json": components["schemas"]["octotify_pkg_response.Response"];
                    };
                };
            };
        };
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/channels/{id}/enable": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get?: never;
        /**
         * 启用推送渠道
         * @description 启用指定推送渠道，恢复消息推送功能。
         *     ## 使用场景
         *     1. 重新启用之前停用的渠道
         *     2. 恢复某个渠道的消息推送能力
         *     ## 注意事项
         *     - 只能启用已停用的渠道
         *     - 启用后该渠道立即可以接收消息
         *     - 只能启用自己创建的渠道
         *     ## 错误码说明
         *     - 100001: 未提供认证令牌
         *     - 120203: 渠道不存在
         *     - 120204: 无权限操作
         *     - 120207: 渠道已启用
         */
        put: {
            parameters: {
                query?: never;
                header?: never;
                path: {
                    /** @description 渠道ID，在路径中传递 */
                    id: number;
                };
                cookie?: never;
            };
            requestBody?: never;
            responses: {
                /** @description 启用成功 */
                200: {
                    headers: {
                        [name: string]: unknown;
                    };
                    content: {
                        "application/json": components["schemas"]["octotify_pkg_response.Response"];
                    };
                };
            };
        };
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/channels/{id}/test": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get?: never;
        put?: never;
        /**
         * 测试渠道连接
         * @description 发送测试消息到指定渠道，验证渠道配置是否正确。
         *     ## 使用场景
         *     1. 创建渠道后验证配置
         *     2. 修改渠道配置后验证连接
         *     ## 注意事项
         *     - 测试消息会真实推送到目标渠道
         *     - 测试失败时返回详细错误信息
         *     - 只能测试自己创建的渠道
         *     ## 错误码说明
         *     - 100001: 未提供认证令牌
         *     - 120203: 渠道不存在
         *     - 120204: 无权限测试
         *     - 120205: 渠道已停用
         *     - 120206: 测试推送失败
         */
        post: {
            parameters: {
                query?: never;
                header?: never;
                path: {
                    /** @description 渠道ID，在路径中传递 */
                    id: number;
                };
                cookie?: never;
            };
            requestBody?: never;
            responses: {
                /** @description 测试成功 */
                200: {
                    headers: {
                        [name: string]: unknown;
                    };
                    content: {
                        "application/json": components["schemas"]["octotify_pkg_response.Response"];
                    };
                };
            };
        };
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/messages": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /**
         * 查看消息列表
         * @description 查看当前用户的消息记录列表，按创建时间倒序排列。
         *     ## 使用场景
         *     1. 消息管理页面展示消息列表
         *     2. 查看最近推送的消息
         *     ## 注意事项
         *     - 列表按创建时间倒序排列
         *     - 已删除的消息不会出现在列表中
         *     - 返回的消息不包含完整的内容详情
         *     ## 错误码说明
         *     - 100001: 未提供认证令牌
         */
        get: {
            parameters: {
                query?: {
                    /** @description 页码，从 1 开始，默认 1 */
                    page?: number;
                    /** @description 每页条数，默认 20，最大 100 */
                    page_size?: number;
                };
                header?: never;
                path?: never;
                cookie?: never;
            };
            requestBody?: never;
            responses: {
                /** @description 查询成功 */
                200: {
                    headers: {
                        [name: string]: unknown;
                    };
                    content: {
                        "application/json": components["schemas"]["octotify_pkg_response.Response"] & {
                            data?: components["schemas"]["octotify_pkg_response.PageResult"] & {
                                list?: components["schemas"]["octotify_internal_handler_dto.MessageDTO"][];
                            };
                        };
                    };
                };
            };
        };
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/messages/filter": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /**
         * 筛选消息
         * @description 根据来源、渠道、状态、时间范围等条件筛选消息，支持多条件组合查询。
         *     ## 筛选条件说明
         *     - **source_id**: 按消息来源筛选
         *     - **channel_id**: 按推送渠道筛选
         *     - **status**: 按推送状态筛选（100-待推送 200-成功 300-失败）
         *     - **start_date/end_date**: 按时间范围筛选（Unix 毫秒时间戳）
         *     - **keyword**: 按关键词搜索消息标题和内容
         *     ## 使用场景
         *     1. 查看某个来源的所有消息
         *     2. 查看某个渠道的所有消息
         *     3. 查看推送失败的消息
         *     4. 查看某个时间段内的消息
         *     ## 注意事项
         *     - 所有筛选条件都是可选的，可以组合使用
         *     - 多个条件之间是 AND 关系
         *     - 结果按创建时间倒序排列
         *     ## 错误码说明
         *     - 100001: 未提供认证令牌
         */
        get: {
            parameters: {
                query?: {
                    /** @description 来源 ID（可选） */
                    source_id?: number;
                    /** @description 渠道 ID（可选） */
                    channel_id?: number;
                    /** @description 推送状态（可选）：100-待推送 200-成功 300-失败 */
                    status?: number;
                    /** @description 开始时间（可选，Unix 毫秒时间戳） */
                    start_date?: number;
                    /** @description 结束时间（可选，Unix 毫秒时间戳） */
                    end_date?: number;
                    /** @description 关键词（可选，搜索标题和内容） */
                    keyword?: string;
                    /** @description 页码，从 1 开始，默认 1 */
                    page?: number;
                    /** @description 每页条数，默认 20，最大 100 */
                    page_size?: number;
                };
                header?: never;
                path?: never;
                cookie?: never;
            };
            requestBody?: never;
            responses: {
                /** @description 查询成功 */
                200: {
                    headers: {
                        [name: string]: unknown;
                    };
                    content: {
                        "application/json": components["schemas"]["octotify_pkg_response.Response"] & {
                            data?: components["schemas"]["octotify_pkg_response.PageResult"] & {
                                list?: components["schemas"]["octotify_internal_handler_dto.MessageDTO"][];
                            };
                        };
                    };
                };
            };
        };
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/messages/{id}": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /**
         * 查看消息详情
         * @description 查看单条消息的详细信息，包含消息内容、来源名称、渠道名称和推送状态。
         *     ## 使用场景
         *     1. 消息详情页展示
         *     2. 查看消息的完整内容
         *     3. 查看消息的推送状态
         *     ## 注意事项
         *     - 只能查看自己用户下的消息
         *     - 包含完整的消息内容
         *     - 包含来源和渠道的详细信息
         *     ## 错误码说明
         *     - 100001: 未提供认证令牌
         *     - 120300: 消息不存在
         *     - 120301: 无权限查看
         */
        get: {
            parameters: {
                query?: never;
                header?: never;
                path: {
                    /** @description 消息ID，在路径中传递 */
                    id: number;
                };
                cookie?: never;
            };
            requestBody?: never;
            responses: {
                /** @description 查询成功 */
                200: {
                    headers: {
                        [name: string]: unknown;
                    };
                    content: {
                        "application/json": components["schemas"]["octotify_pkg_response.Response"] & {
                            data?: components["schemas"]["octotify_internal_handler_dto.MessageDetailDTO"];
                        };
                    };
                };
            };
        };
        put?: never;
        post?: never;
        /**
         * 删除消息
         * @description 删除单条消息记录（软删除），删除后消息不再显示在列表中。
         *     ## 使用场景
         *     1. 清理不需要的消息记录
         *     2. 删除敏感消息
         *     ## 注意事项
         *     - 删除是软删除，数据库中标记为已删除状态
         *     - 只能删除自己用户下的消息
         *     - 删除后无法恢复
         *     ## 错误码说明
         *     - 100001: 未提供认证令牌
         *     - 120300: 消息不存在
         *     - 120302: 无权限删除
         *     - 120303: 删除消息失败
         */
        delete: {
            parameters: {
                query?: never;
                header?: never;
                path: {
                    /** @description 消息ID，在路径中传递 */
                    id: number;
                };
                cookie?: never;
            };
            requestBody?: never;
            responses: {
                /** @description 删除成功 */
                200: {
                    headers: {
                        [name: string]: unknown;
                    };
                    content: {
                        "application/json": components["schemas"]["octotify_pkg_response.Response"];
                    };
                };
            };
        };
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/push/{token}": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get?: never;
        put?: never;
        /**
         * 推送消息
         * @description 外部系统通过 Source Token 推送消息到平台，消息会被并发推送到所有绑定的有效渠道。
         *     ## 认证方式
         *     使用 Source Token 进行认证，而非用户 Access Token。
         *     Token 格式：src{uuid}，例如：src0196a3b2c4d50000a1b2c3d4e5f67890
         *     ## 推送流程
         *     1. 系统验证 Source Token 有效性
         *     2. 查找该来源绑定的所有启用状态的渠道
         *     3. 并发推送到所有渠道
         *     4. 返回各渠道的推送结果
         *     ## 使用场景
         *     1. CI/CD 流水线推送构建结果
         *     2. 监控系统推送告警信息
         *     3. 业务系统推送通知消息
         *     ## 注意事项
         *     - Source Token 通过 URL 路径参数传递
         *     - 消息会并发推送到所有绑定的有效渠道
         *     - 返回结果包含每个渠道的推送状态
         *     - 推送失败不影响其他渠道的推送
         *     - 来源被停用后无法推送消息
         *     ## 错误码说明
         *     - 120400: 来源 Token 无效
         *     - 120401: 来源已停用
         *     - 120402: 来源不存在
         *     - 120403: 没有可用的推送渠道
         *     - 120404: 推送消息失败
         */
        post: {
            parameters: {
                query?: never;
                header?: never;
                path: {
                    /** @description 来源 Token，格式为 src{uuid} */
                    token: string;
                };
                cookie?: never;
            };
            /** @description 推送消息请求参数 */
            requestBody: {
                content: {
                    "application/json": components["schemas"]["octotify_internal_handler_dto.PushMessageReq"];
                };
            };
            responses: {
                /** @description 推送成功 */
                200: {
                    headers: {
                        [name: string]: unknown;
                    };
                    content: {
                        "application/json": components["schemas"]["octotify_pkg_response.Response"] & {
                            data?: components["schemas"]["octotify_internal_handler_dto.PushResponse"];
                        };
                    };
                };
            };
        };
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/sources": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /**
         * 查看消息来源列表
         * @description 分页查询当前用户的所有消息来源列表，返回来源基本信息（不包含 Token）。
         *     ## 使用场景
         *     1. 来源管理页面展示来源列表
         *     2. 下拉选择器获取来源选项
         *     ## 注意事项
         *     - 列表按创建时间倒序排列
         *     - 返回的来源不包含 Token 字段
         *     - 已删除的来源不会出现在列表中
         *     ## 错误码说明
         *     - 100001: 未提供认证令牌
         */
        get: {
            parameters: {
                query?: {
                    /** @description 页码，从 1 开始，默认 1 */
                    page?: number;
                    /** @description 每页条数，默认 20，最大 100 */
                    page_size?: number;
                };
                header?: never;
                path?: never;
                cookie?: never;
            };
            requestBody?: never;
            responses: {
                /** @description 查询成功 */
                200: {
                    headers: {
                        [name: string]: unknown;
                    };
                    content: {
                        "application/json": components["schemas"]["octotify_pkg_response.Response"] & {
                            data?: components["schemas"]["octotify_pkg_response.PageResult"] & {
                                list?: components["schemas"]["octotify_internal_handler_dto.SourceDTO"][];
                            };
                        };
                    };
                };
            };
        };
        put?: never;
        /**
         * 创建消息来源
         * @description 创建一个新的消息来源，系统自动生成 Source Token，用于外部系统推送消息。
         *     ## 使用场景
         *     1. CI/CD 流水线创建为消息来源
         *     2. 监控系统创建为消息来源
         *     3. 业务系统创建为消息来源
         *     ## 注意事项
         *     - Source Token 仅在创建时返回一次，请妥善保存
         *     - 如忘记 Token，可通过查看令牌接口获取（需密码验证）
         *     - 来源创建后默认启用状态
         *     - 每个来源可绑定多个推送渠道
         *     ## 错误码说明
         *     - 100001: 未提供认证令牌
         *     - 120100: 来源名称已存在
         *     - 120101: 创建来源失败
         */
        post: {
            parameters: {
                query?: never;
                header?: never;
                path?: never;
                cookie?: never;
            };
            /** @description 创建消息来源请求参数 */
            requestBody: {
                content: {
                    "application/json": components["schemas"]["octotify_internal_handler_dto.CreateSourceReq"];
                };
            };
            responses: {
                /** @description 创建成功 */
                200: {
                    headers: {
                        [name: string]: unknown;
                    };
                    content: {
                        "application/json": components["schemas"]["octotify_pkg_response.Response"] & {
                            data?: components["schemas"]["octotify_internal_handler_dto.SourceDTO"];
                        };
                    };
                };
            };
        };
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/sources/{id}": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /**
         * 查看来源详情
         * @description 查询指定消息来源的详细信息，包含来源 Token、使用时间和已绑定的有效渠道列表。
         *     ## 使用场景
         *     1. 来源详情页展示
         *     2. 查看来源绑定的渠道
         *     3. 查看来源的推送 Token
         *     ## 注意事项
         *     - 返回的 Token 可用于推送消息接口
         *     - 渠道列表只包含启用状态的渠道
         *     - 只能查看自己创建的来源
         *     ## 错误码说明
         *     - 100001: 未提供认证令牌
         *     - 120102: 来源不存在
         *     - 120103: 无权限查看
         */
        get: {
            parameters: {
                query?: never;
                header?: never;
                path: {
                    /** @description 来源ID，在路径中传递 */
                    id: number;
                };
                cookie?: never;
            };
            requestBody?: never;
            responses: {
                /** @description 查询成功 */
                200: {
                    headers: {
                        [name: string]: unknown;
                    };
                    content: {
                        "application/json": components["schemas"]["octotify_pkg_response.Response"] & {
                            data?: components["schemas"]["octotify_internal_handler_dto.SourceDetailResponse"];
                        };
                    };
                };
            };
        };
        /**
         * 编辑消息来源
         * @description 编辑消息来源的名称和描述，不影响已绑定的渠道和推送功能。
         *     ## 使用场景
         *     1. 修改来源名称使其更易识别
         *     2. 更新来源描述信息
         *     ## 注意事项
         *     - 只能编辑自己创建的来源
         *     - 名称不能与其他来源重复
         *     - 编辑后不影响 Source Token
         *     ## 错误码说明
         *     - 100001: 未提供认证令牌
         *     - 120100: 来源名称已存在
         *     - 120102: 来源不存在
         *     - 120103: 无权限编辑
         */
        put: {
            parameters: {
                query?: never;
                header?: never;
                path: {
                    /** @description 来源ID，在路径中传递 */
                    id: number;
                };
                cookie?: never;
            };
            /** @description 编辑消息来源请求参数 */
            requestBody: {
                content: {
                    "application/json": components["schemas"]["octotify_internal_handler_dto.UpdateSourceReq"];
                };
            };
            responses: {
                /** @description 编辑成功 */
                200: {
                    headers: {
                        [name: string]: unknown;
                    };
                    content: {
                        "application/json": components["schemas"]["octotify_pkg_response.Response"];
                    };
                };
            };
        };
        post?: never;
        /**
         * 删除消息来源
         * @description 软删除指定消息来源及其关联渠道关系，需要密码二次验证。删除后来源和所有关联数据不可恢复。
         *     ## 使用场景
         *     1. 永久删除不再使用的来源
         *     2. 清理测试数据
         *     ## 注意事项
         *     - 删除是软删除，数据库中标记为已删除状态
         *     - 删除后该来源的 Token 立即失效
         *     - 关联的渠道关系也会被删除
         *     - 只能删除自己创建的来源
         *     - 注意：DELETE 请求需在 Body 中传递密码（{"password": "xxx"}），主流网关均支持此用法
         *     ## 错误码说明
         *     - 100001: 未提供认证令牌
         *     - 110200: 密码错误
         *     - 120102: 来源不存在
         *     - 120103: 无权限操作
         *     - 120107: 删除来源失败
         */
        delete: {
            parameters: {
                query?: never;
                header?: never;
                path: {
                    /** @description 来源ID，在路径中传递 */
                    id: number;
                };
                cookie?: never;
            };
            /** @description 密码验证请求参数 */
            requestBody: {
                content: {
                    "application/json": components["schemas"]["octotify_internal_handler_dto.VerifyPasswordReq"];
                };
            };
            responses: {
                /** @description 删除成功 */
                200: {
                    headers: {
                        [name: string]: unknown;
                    };
                    content: {
                        "application/json": components["schemas"]["octotify_pkg_response.Response"];
                    };
                };
            };
        };
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/sources/{id}/disable": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get?: never;
        /**
         * 停用消息来源
         * @description 停用指定消息来源，需要密码二次验证，停用后该来源无法推送消息。
         *     ## 使用场景
         *     1. 临时停止某个来源的消息推送
         *     2. 来源不再使用时停用而非删除
         *     ## 注意事项
         *     - 停用后使用该 Token 推送的消息会被拒绝
         *     - 已推送的历史消息不受影响
         *     - 可以随时重新启用
         *     - 只能停用自己创建的来源
         *     ## 错误码说明
         *     - 100001: 未提供认证令牌
         *     - 110200: 密码错误
         *     - 120102: 来源不存在
         *     - 120103: 无权限操作
         *     - 120105: 来源已停用
         */
        put: {
            parameters: {
                query?: never;
                header?: never;
                path: {
                    /** @description 来源ID，在路径中传递 */
                    id: number;
                };
                cookie?: never;
            };
            /** @description 密码验证请求参数 */
            requestBody: {
                content: {
                    "application/json": components["schemas"]["octotify_internal_handler_dto.VerifyPasswordReq"];
                };
            };
            responses: {
                /** @description 停用成功 */
                200: {
                    headers: {
                        [name: string]: unknown;
                    };
                    content: {
                        "application/json": components["schemas"]["octotify_pkg_response.Response"];
                    };
                };
            };
        };
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/sources/{id}/enable": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get?: never;
        /**
         * 启用消息来源
         * @description 启用指定消息来源，需要密码二次验证，恢复消息推送功能。
         *     ## 使用场景
         *     1. 重新启用之前停用的来源
         *     2. 恢复某个来源的消息推送能力
         *     ## 注意事项
         *     - 只能启用已停用的来源
         *     - 启用后该来源的 Token 立即生效
         *     - 只能启用自己创建的来源
         *     ## 错误码说明
         *     - 100001: 未提供认证令牌
         *     - 110200: 密码错误
         *     - 120102: 来源不存在
         *     - 120103: 无权限操作
         *     - 120106: 来源已启用
         */
        put: {
            parameters: {
                query?: never;
                header?: never;
                path: {
                    /** @description 来源ID，在路径中传递 */
                    id: number;
                };
                cookie?: never;
            };
            /** @description 密码验证请求参数 */
            requestBody: {
                content: {
                    "application/json": components["schemas"]["octotify_internal_handler_dto.VerifyPasswordReq"];
                };
            };
            responses: {
                /** @description 启用成功 */
                200: {
                    headers: {
                        [name: string]: unknown;
                    };
                    content: {
                        "application/json": components["schemas"]["octotify_pkg_response.Response"];
                    };
                };
            };
        };
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/sources/{id}/token": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get?: never;
        /**
         * 重置来源令牌
         * @description 重置指定消息来源的 Token，需要密码二次验证，旧 Token 立即失效。
         *     ## 使用场景
         *     1. Token 泄露时紧急重置
         *     2. 定期更换 Token 以提高安全性
         *     ## 注意事项
         *     - 重置后旧 Token 立即失效，使用旧 Token 推送会失败
         *     - 必须更新所有使用旧 Token 的外部系统配置
         *     - 新 Token 仅在重置时返回一次，请妥善保存
         *     - 只能重置自己创建的来源
         *     ## 错误码说明
         *     - 100001: 未提供认证令牌
         *     - 110200: 密码错误
         *     - 120102: 来源不存在
         *     - 120103: 无权限操作
         *     - 120104: 重置令牌失败
         */
        put: {
            parameters: {
                query?: never;
                header?: never;
                path: {
                    /** @description 来源ID，在路径中传递 */
                    id: number;
                };
                cookie?: never;
            };
            /** @description 密码验证请求参数 */
            requestBody: {
                content: {
                    "application/json": components["schemas"]["octotify_internal_handler_dto.VerifyPasswordReq"];
                };
            };
            responses: {
                /** @description 重置成功 */
                200: {
                    headers: {
                        [name: string]: unknown;
                    };
                    content: {
                        "application/json": components["schemas"]["octotify_pkg_response.Response"] & {
                            data?: components["schemas"]["octotify_internal_handler_dto.SourceTokenResponse"];
                        };
                    };
                };
            };
        };
        /**
         * 查看来源令牌
         * @description 查询指定消息来源的 Token，需要密码二次验证以确保安全性。
         *     ## 使用场景
         *     1. 忘记 Token 时查看
         *     2. 配置外部系统推送时获取 Token
         *     ## 注意事项
         *     - 必须提供正确的用户密码才能查看
         *     - Token 格式为 src{uuid}，例如：src0196a3b2c4d50000a1b2c3d4e5f67890
         *     - 只能查看自己创建的来源
         *     ## 错误码说明
         *     - 100001: 未提供认证令牌
         *     - 110200: 密码错误
         *     - 120102: 来源不存在
         *     - 120103: 无权限查看
         */
        post: {
            parameters: {
                query?: never;
                header?: never;
                path: {
                    /** @description 来源ID，在路径中传递 */
                    id: number;
                };
                cookie?: never;
            };
            /** @description 密码验证请求参数 */
            requestBody: {
                content: {
                    "application/json": components["schemas"]["octotify_internal_handler_dto.VerifyPasswordReq"];
                };
            };
            responses: {
                /** @description 查看成功 */
                200: {
                    headers: {
                        [name: string]: unknown;
                    };
                    content: {
                        "application/json": components["schemas"]["octotify_pkg_response.Response"] & {
                            data?: components["schemas"]["octotify_internal_handler_dto.SourceTokenResponse"];
                        };
                    };
                };
            };
        };
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/user/password": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get?: never;
        /**
         * 修改密码
         * @description 修改当前用户的登录密码，修改成功后所有 Refresh Token 将被撤销，需要重新登录。
         *     ## 密码要求
         *     - 8-128 个字符
         *     - 必须包含小写字母
         *     - 必须包含大写字母
         *     - 必须包含数字
         *     ## 使用场景
         *     1. 用户定期修改密码
         *     2. 用户怀疑密码泄露时修改密码
         *     ## 注意事项
         *     - 修改密码后所有已登录的设备都需要重新登录
         *     - 旧密码必须正确才能修改
         *     - 新密码不能与旧密码相同
         *     ## 错误码说明
         *     - 100001: 未提供认证令牌
         *     - 110200: 旧密码错误
         *     - 110102: 修改密码失败
         */
        put: {
            parameters: {
                query?: never;
                header?: never;
                path?: never;
                cookie?: never;
            };
            /** @description 修改密码请求参数 */
            requestBody: {
                content: {
                    "application/json": components["schemas"]["octotify_internal_handler_dto.ChangePasswordReq"];
                };
            };
            responses: {
                /** @description 修改成功 */
                200: {
                    headers: {
                        [name: string]: unknown;
                    };
                    content: {
                        "application/json": components["schemas"]["octotify_pkg_response.Response"];
                    };
                };
            };
        };
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/user/profile": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /**
         * 查询用户信息
         * @description 获取当前登录用户的个人信息，包括用户 ID、用户名和创建时间。
         *     ## 使用场景
         *     1. 页面加载时获取用户信息展示在导航栏
         *     2. 用户中心页面展示个人信息
         *     ## 注意事项
         *     - 需要提供有效的 Access Token
         *     - 只能查询自己的信息，不能查询其他用户
         *     ## 错误码说明
         *     - 100001: 未提供认证令牌或认证令牌无效或已过期
         *     - 110103: 用户不存在
         */
        get: {
            parameters: {
                query?: never;
                header?: never;
                path?: never;
                cookie?: never;
            };
            requestBody?: never;
            responses: {
                /** @description 查询成功 */
                200: {
                    headers: {
                        [name: string]: unknown;
                    };
                    content: {
                        "application/json": components["schemas"]["octotify_pkg_response.Response"] & {
                            data?: components["schemas"]["octotify_internal_handler_dto.UserProfileResp"];
                        };
                    };
                };
            };
        };
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/user/register": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get?: never;
        put?: never;
        /**
         * 用户注册
         * @description 注册新用户账号，注册成功后自动登录并返回 Access Token 和 Refresh Token。
         *     ## 密码要求
         *     - 8-128 个字符
         *     - 必须包含小写字母
         *     - 必须包含大写字母
         *     - 必须包含数字
         *     ## 使用场景
         *     1. 新用户首次创建账号
         *     2. 管理员为用户创建账号
         *     ## 注意事项
         *     - 用户名唯一，重复注册会返回错误
         *     - 注册成功后请妥善保存返回的 Token
         *     - 密码加密存储，系统无法查看原始密码
         *     ## 错误码说明
         *     - 110100: 用户名已存在
         *     - 110101: 注册失败
         */
        post: {
            parameters: {
                query?: never;
                header?: never;
                path?: never;
                cookie?: never;
            };
            /** @description 注册请求参数 */
            requestBody: {
                content: {
                    "application/json": components["schemas"]["octotify_internal_handler_dto.RegisterReq"];
                };
            };
            responses: {
                /** @description 注册成功 */
                200: {
                    headers: {
                        [name: string]: unknown;
                    };
                    content: {
                        "application/json": components["schemas"]["octotify_pkg_response.Response"] & {
                            data?: components["schemas"]["octotify_internal_handler_dto.AuthResp"];
                        };
                    };
                };
            };
        };
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
}
export type webhooks = Record<string, never>;
export interface components {
    schemas: {
        /** @description 认证成功后返回的 Access Token、Refresh Token 和用户信息 */
        "octotify_internal_handler_dto.AuthResp": {
            /**
             * @description 访问令牌，有效期 1 小时
             * @example eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9...
             */
            access_token?: string;
            /**
             * @description 刷新令牌，有效期 7 天
             * @example eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9...
             */
            refresh_token?: string;
            /** @description 用户信息 */
            user?: components["schemas"]["octotify_internal_handler_dto.UserDTO"];
        };
        "octotify_internal_handler_dto.BindCredentialsDTO": {
            /** @description 加密后的 Bot Token（密文） */
            bot_token_ciphertext?: string;
            /** @description Token 加密使用的 Nonce */
            bot_token_nonce?: string;
            /** @description iLink 平台分配的 Bot ID */
            ilink_bot_id?: string;
            /** @description iLink 平台的用户 ID */
            ilink_user_id?: string;
        };
        "octotify_internal_handler_dto.BindStatusResp": {
            /** @description 绑定成功时返回的凭证 */
            credentials?: components["schemas"]["octotify_internal_handler_dto.BindCredentialsDTO"];
            /** @description 绑定状态：pending/scanned/confirmed/expired */
            status?: string;
        };
        /** @description 修改用户登录密码 */
        "octotify_internal_handler_dto.ChangePasswordReq": {
            /**
             * @description 新密码，8-128 个字符，需包含大小写字母和数字
             * @example NewP@ssw0rd123
             */
            new_password: string;
            /**
             * @description 旧密码
             * @example OldP@ssw0rd123
             */
            old_password: string;
        };
        "octotify_internal_handler_dto.ChannelConfig": {
            [key: string]: unknown;
        };
        /** @description 推送渠道的详细信息 */
        "octotify_internal_handler_dto.ChannelDTO": {
            /** @description 渠道配置，JSON 格式 */
            config?: components["schemas"]["octotify_internal_handler_dto.ChannelConfig"];
            /**
             * @description 创建时间（Unix 毫秒时间戳）
             * @example 1714636800000
             */
            created_at?: number;
            /**
             * @description 渠道 ID
             * @example 1
             */
            id?: number;
            /**
             * @description 最后使用时间（Unix 毫秒时间戳，0 表示未使用）
             * @example 1714636800000
             */
            last_used_at?: number;
            /**
             * @description 渠道名称
             * @example 钉钉-运维群
             */
            name?: string;
            /**
             * @description 状态：1-启用 2-停用 -1-已删除
             * @example 1
             */
            status?: number;
            /**
             * @description 渠道类型：wechat, telegram, dingtalk, email, webhook
             * @example dingtalk
             */
            type?: string;
            /**
             * @description 更新时间（Unix 毫秒时间戳）
             * @example 1714636800000
             */
            updated_at?: number;
            /**
             * @description 所属用户 ID
             * @example 1
             */
            user_id?: number;
        };
        "octotify_internal_handler_dto.ChannelTypeMeta": {
            /** @description 配置字段定义 */
            config_fields?: components["schemas"]["octotify_internal_handler_dto.ConfigField"][];
            /** @description 描述 */
            description?: string;
            /** @description 渠道类型名称 */
            name?: string;
            /** @description 渠道类型标识 */
            type?: string;
        };
        "octotify_internal_handler_dto.ConfigField": {
            /** @description 字段标签 */
            label?: string;
            /** @description 字段名称 */
            name?: string;
            /** @description 占位符提示 */
            placeholder?: string;
            /** @description 是否必填 */
            required?: boolean;
            /** @description 字段类型：string, number, password, url */
            type?: string;
        };
        /** @description 创建新的推送渠道，需要指定渠道类型、名称和配置信息 ## 渠道配置示例 - wechat: {"webhook": "https://qyapi.weixin.qq.com/cgi-bin/webhook/send?key=xxx"} - telegram: {"bot_token": "xxx", "chat_id": "xxx"} - dingtalk: {"webhook": "https://oapi.dingtalk.com/robot/send?access_token=xxx"} - email: {"smtp_host": "smtp.example.com", "smtp_port": 587, "username": "xxx", "password": "xxx", "to": "xxx@example.com"} - webhook: {"url": "https://example.com/webhook", "method": "POST", "headers": {"Content-Type": "application/json"}} - feishu: {"webhook_url": "https://open.feishu.cn/open-apis/bot/v2/hook/xxx", "secret": ""} */
        "octotify_internal_handler_dto.CreateChannelReq": {
            /** @description 渠道配置，JSON 对象，不同渠道类型配置不同 */
            config: components["schemas"]["octotify_internal_handler_dto.ChannelConfig"];
            /**
             * @description 渠道名称，1-128 个字符
             * @example 钉钉-运维群
             */
            name: string;
            /**
             * @description 渠道类型，可选值：wechat, telegram, dingtalk, email, webhook
             * @example dingtalk
             */
            type: string;
        };
        /** @description 创建新的消息来源，系统自动生成 Source Token */
        "octotify_internal_handler_dto.CreateSourceReq": {
            /**
             * @description 关联的渠道ID列表
             * @example [
             *       1,
             *       2,
             *       3
             *     ]
             */
            channel_ids?: number[];
            /**
             * @description 来源描述，最多 512 个字符
             * @example GitHub Actions 构建通知
             */
            description?: string;
            /**
             * @description 来源名称，1-128 个字符
             * @example CI Pipeline
             */
            name: string;
        };
        "octotify_internal_handler_dto.GetBindStatusReq": {
            /** @description iLink 返回的二维码原始值 */
            qrcode: string;
        };
        /** @description 用户登录时使用的请求参数 */
        "octotify_internal_handler_dto.LoginReq": {
            /**
             * @description 密码，8-128 个字符，需包含大小写字母和数字
             * @example P@ssw0rd123
             */
            password: string;
            /**
             * @description 用户名，3-64 个字符，仅允许字母、数字和下划线
             * @example octotify
             */
            username: string;
        };
        /** @description 消息列表中的单条消息记录 */
        "octotify_internal_handler_dto.MessageDTO": {
            /**
             * @description 渠道 ID
             * @example 1
             */
            channel_id?: number;
            /**
             * @description 消息内容
             * @example Build #123 passed
             */
            content?: string;
            /**
             * @description 创建时间（Unix 毫秒时间戳）
             * @example 1714636800000
             */
            created_at?: number;
            /**
             * @description 消息 ID
             * @example 1
             */
            id?: number;
            /**
             * @description 来源 ID
             * @example 1
             */
            source_id?: number;
            /**
             * @description 推送状态：100-待推送 200-成功 300-失败 -1-已删除
             * @example 200
             */
            status?: number;
            /**
             * @description 消息标题
             * @example CI Build
             */
            title?: string;
            /**
             * @description 更新时间（Unix 毫秒时间戳）
             * @example 1714636800000
             */
            updated_at?: number;
        };
        /** @description 单条消息的详细信息，包含来源名称、渠道名称和类型 */
        "octotify_internal_handler_dto.MessageDetailDTO": {
            /**
             * @description 渠道 ID
             * @example 1
             */
            channel_id?: number;
            /**
             * @description 渠道名称
             * @example 钉钉-运维群
             */
            channel_name?: string;
            /**
             * @description 渠道类型：wechat, telegram, dingtalk, email, webhook
             * @example dingtalk
             */
            channel_type?: string;
            /**
             * @description 消息内容
             * @example Build #123 passed
             */
            content?: string;
            /**
             * @description 创建时间（Unix 毫秒时间戳）
             * @example 1714636800000
             */
            created_at?: number;
            /**
             * @description 消息 ID
             * @example 1
             */
            id?: number;
            /**
             * @description 来源 ID
             * @example 1
             */
            source_id?: number;
            /**
             * @description 来源名称
             * @example CI Pipeline
             */
            source_name?: string;
            /**
             * @description 推送状态：100-待推送 200-成功 300-失败 -1-已删除
             * @example 200
             */
            status?: number;
            /**
             * @description 消息标题
             * @example CI Build
             */
            title?: string;
            /**
             * @description 更新时间（Unix 毫秒时间戳）
             * @example 1714636800000
             */
            updated_at?: number;
        };
        /** @description 外部系统通过 Source Token 推送消息时使用的请求参数 */
        "octotify_internal_handler_dto.PushMessageReq": {
            /**
             * @description 消息内容，不能为空
             * @example Build #123 passed
             */
            message: string;
            /**
             * @description 消息标题，不能为空
             * @example CI Build
             */
            title: string;
        };
        /** @description 消息推送接口的返回结果，包含各渠道的推送详情 */
        "octotify_internal_handler_dto.PushResponse": {
            /** @description 推送失败数 */
            failed?: number;
            /** @description 各渠道推送结果详情 */
            results?: components["schemas"]["octotify_internal_handler_dto.PushResult"][];
            /** @description 推送成功数 */
            success?: number;
            /** @description 推送渠道总数 */
            total?: number;
        };
        /** @description 并发推送到多个渠道时，单个渠道的推送结果 */
        "octotify_internal_handler_dto.PushResult": {
            /** @description 渠道 ID */
            channel_id?: number;
            /** @description 渠道名称 */
            channel_name?: string;
            /** @description 失败时的错误信息 */
            error?: string;
            /** @description 消息 ID */
            message_id?: number;
            /** @description 是否推送成功 */
            success?: boolean;
        };
        /** @description 使用 Refresh Token 获取新的 Access Token */
        "octotify_internal_handler_dto.RefreshReq": {
            /**
             * @description Refresh Token，登录时获取
             * @example eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9...
             */
            refresh_token: string;
        };
        /** @description 注册新用户时使用的请求参数 */
        "octotify_internal_handler_dto.RegisterReq": {
            /**
             * @description 密码，8-128 个字符，需包含大小写字母和数字
             * @example P@ssw0rd123
             */
            password: string;
            /**
             * @description 用户名，3-64 个字符，仅允许字母、数字和下划线
             * @example octotify
             */
            username: string;
        };
        /** @description 消息来源的基本信息，不包含 Token */
        "octotify_internal_handler_dto.SourceDTO": {
            /**
             * @description 创建时间（Unix 毫秒时间戳）
             * @example 1714636800000
             */
            created_at?: number;
            /**
             * @description 来源描述
             * @example GitHub Actions 构建通知
             */
            description?: string;
            /**
             * @description 来源 ID
             * @example 1
             */
            id?: number;
            /**
             * @description 来源名称
             * @example CI Pipeline
             */
            name?: string;
            /**
             * @description 状态：1-启用 2-停用 -1-已删除
             * @example 1
             */
            status?: number;
            /**
             * @description 推送 Token（仅创建时返回）
             * @example src0196a3b2c4d50000a1b2c3d4e5f67890
             */
            token?: string;
            /**
             * @description 所属用户 ID
             * @example 1
             */
            user_id?: number;
        };
        /** @description 消息来源的详细信息，包含 Token 和使用时间 */
        "octotify_internal_handler_dto.SourceDetailDTO": {
            /**
             * @description 创建时间（Unix 毫秒时间戳）
             * @example 1714636800000
             */
            created_at?: number;
            /**
             * @description 来源描述
             * @example GitHub Actions 构建通知
             */
            description?: string;
            /**
             * @description 来源 ID
             * @example 1
             */
            id?: number;
            /**
             * @description 最后使用时间（Unix 毫秒时间戳，0 表示未使用）
             * @example 1714636800000
             */
            last_used_at?: number;
            /**
             * @description 来源名称
             * @example CI Pipeline
             */
            name?: string;
            /**
             * @description 状态：1-启用 2-停用 -1-已删除
             * @example 1
             */
            status?: number;
            /**
             * @description 推送 Token
             * @example src0196a3b2c4d50000a1b2c3d4e5f67890
             */
            token?: string;
            /**
             * @description 更新时间（Unix 毫秒时间戳）
             * @example 1714636800000
             */
            updated_at?: number;
            /**
             * @description 所属用户 ID
             * @example 1
             */
            user_id?: number;
        };
        /** @description 来源详情及其已绑定的有效渠道列表 */
        "octotify_internal_handler_dto.SourceDetailResponse": {
            /** @description 已绑定渠道列表 */
            channels?: components["schemas"]["octotify_internal_handler_dto.ChannelDTO"][];
            /** @description 来源详情 */
            source?: components["schemas"]["octotify_internal_handler_dto.SourceDetailDTO"];
        };
        /** @description 查看或重置来源令牌时返回的 Token 值 */
        "octotify_internal_handler_dto.SourceTokenResponse": {
            /**
             * @description 推送 Token
             * @example src0196a3b2c4d50000a1b2c3d4e5f67890
             */
            token?: string;
        };
        "octotify_internal_handler_dto.StartBindResp": {
            /** @description 二维码原始值，用于后续查询绑定状态 */
            qrcode?: string;
            /** @description 二维码 URL，用户扫码使用 */
            qrcode_url?: string;
        };
        /** @description 编辑已有推送渠道的名称和配置信息 ## 渠道配置示例 - wechat: {"webhook": "https://qyapi.weixin.qq.com/cgi-bin/webhook/send?key=xxx"} - telegram: {"bot_token": "xxx", "chat_id": "xxx"} - dingtalk: {"webhook": "https://oapi.dingtalk.com/robot/send?access_token=xxx"} - email: {"smtp_host": "smtp.example.com", "smtp_port": 587, "username": "xxx", "password": "xxx", "to": "xxx@example.com"} - webhook: {"url": "https://example.com/webhook", "method": "POST", "headers": {"Content-Type": "application/json"}} */
        "octotify_internal_handler_dto.UpdateChannelReq": {
            /** @description 渠道配置，JSON 对象，不同渠道类型配置不同 */
            config: components["schemas"]["octotify_internal_handler_dto.ChannelConfig"];
            /**
             * @description 渠道名称，1-128 个字符
             * @example 钉钉-运维群
             */
            name: string;
        };
        /** @description 编辑已有消息来源的名称、描述和绑定渠道 */
        "octotify_internal_handler_dto.UpdateSourceReq": {
            /**
             * @description 关联的渠道ID列表
             * @example [
             *       1,
             *       2,
             *       3
             *     ]
             */
            channel_ids?: number[];
            /**
             * @description 来源描述，最多 512 个字符
             * @example GitHub Actions 构建通知
             */
            description?: string;
            /**
             * @description 来源名称，1-128 个字符
             * @example CI Pipeline
             */
            name: string;
        };
        /** @description 用户基本信息，用于登录/注册/刷新令牌等接口返回 */
        "octotify_internal_handler_dto.UserDTO": {
            /**
             * @description 创建时间（Unix 时间戳）
             * @example 1714636800
             */
            created_at?: number;
            /**
             * @description 用户 ID
             * @example 1
             */
            id?: number;
            /**
             * @description 用户名
             * @example octotify
             */
            username?: string;
        };
        /** @description 查询用户个人信息时返回的响应结构 */
        "octotify_internal_handler_dto.UserProfileResp": {
            /** @description 用户信息 */
            user?: components["schemas"]["octotify_internal_handler_dto.UserDTO"];
        };
        /** @description 需要密码二次验证的接口（查看令牌、重置令牌、停用/启用/删除来源） */
        "octotify_internal_handler_dto.VerifyPasswordReq": {
            /**
             * @description 用户密码
             * @example P@ssw0rd123
             */
            password: string;
        };
        /** @description 所有分页接口返回的数据结构 */
        "octotify_pkg_response.PageResult": {
            /** @description 数据列表 */
            list?: unknown;
            /** @description 当前页码 */
            page?: number;
            /** @description 每页条数 */
            page_size?: number;
            /** @description 总记录数 */
            total?: number;
        };
        /** @description 所有 API 接口返回的统一格式，包含业务状态码、消息和数据 */
        "octotify_pkg_response.Response": {
            /** @description 业务状态码，0 表示成功，非 0 表示失败 */
            code?: number;
            /** @description 业务数据，成功时返回业务数据，失败时可能为空或错误详情 */
            data?: unknown;
            /** @description 提示信息，成功时为"请求成功"，失败时为错误描述 */
            msg?: string;
        };
    };
    responses: never;
    parameters: never;
    requestBodies: never;
    headers: never;
    pathItems: never;
}
export type $defs = Record<string, never>;
export type operations = Record<string, never>;
