...
type PushMessageReq struct {
	Title   string `json:"title" binding:"required,min=1,max=256" example:"CI Build"`    // 消息标题
	Message string `json:"message" binding:"required,min=1" example:"Build #123 passed"` // 消息内容
}
...
type MessageDetailDTO struct {
	...
	ChannelType string `json:"channel_type" example:"dingtalk"`     // 渠道类型
	Title       string `json:"title" example:"CI Build"`            // 消息标题
	Content     string `json:"content" example:"Build #123 passed"` // 消息内容
	Status      int    `json:"status" example:"200"`                // 推送状态
	CreatedAt   int64  `json:"created_at" example:"1714636800000"`  // 创建时间（Unix 毫秒时间戳）
	UpdatedAt   int64  `json:"updated_at" example:"1714636800000"`  // 更新时间（Unix 毫秒时间戳）
}
...
type MessageFilterReq struct {
	PageReq
	SourceID  *int64 `form:"source_id" example:"1"`              // 来源 ID
	ChannelID *int64 `form:"channel_id" example:"1"`             // 渠道 ID
	Status    *int   `form:"status" example:"200"`               // 推送状态
	StartDate *int64 `form:"start_date" example:"1714636800000"` // 开始时间（Unix 毫秒）
	EndDate   *int64 `form:"end_date" example:"1714723200000"`   // 结束时间（Unix 毫秒）
	Keyword   string `form:"keyword" example:"Build"`            // 关键词（搜索标题和内容）
}
...
type PushResponse struct {
	...
	Results []*PushResult `json:"results"` // 各渠道推送结果
}
