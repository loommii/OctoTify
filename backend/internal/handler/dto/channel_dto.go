...
type CreateChannelReq struct {
	...
	Config datatypes.JSON `json:"config" binding:"required" swaggertype:"object"`          // 渠道配置，JSON 对象，不同渠道类型配置不同
}
...
type ChannelDTO struct {
	...
	Config     datatypes.JSON `json:"config" swaggertype:"object"`          // 渠道配置，JSON 格式
